...
app.post('/api/videos', async (req, res) => {
  const { prompt, aspectRatio = '16:9', duration = '8 seconds', quality = '1080p' } = req.body
  ...
  try {
    ...
    const response = await fetch(`https://api.replicate.com/v1/models/${owner}/${name}/predictions`, {
      ...
      body: JSON.stringify({ input: { prompt: prompt.trim(), aspect_ratio: aspectRatio, duration, quality } }),
    })
