...
function App() {
  ...
  const generateVideo = async () => {
    ...
    setGenerationError('')
    try {
      const videoUrl = await createVideo({ prompt, aspectRatio: aspect, duration, quality, style })
      setVideos([{ title: prompt.trim().slice(0, 32), meta: `${style} · ${aspect} · ${duration} · ${quality}`, color: 'aurora', state: 'Ready', videoUrl }, ...videos])
      setPrompt('')
