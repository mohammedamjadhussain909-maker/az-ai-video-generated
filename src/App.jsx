...
function App() {
  ...
  const generateVideo = async () => {
    ...
    setGenerationError('')
    try {
      const videoUrl = await createVideo({ prompt, aspectRatio: aspect, duration, quality, style, contentType, voice, music })
      setVideos([{ title: prompt.trim().slice(0, 32), meta: `${contentType} · ${style} · ${duration} · ${quality}`, color: 'aurora', state: 'Ready', videoUrl }, ...videos])
      setPrompt('')
