...
function App() {
  ...
  const generateVideo = () => {
    ...
    setTimeout(() => {
      setVideos([{ title: prompt.trim().slice(0, 32), meta: `AI video · ${aspect} · 8 sec`, color: 'aurora', state: 'Just created' }, ...videos])
      setPrompt('')
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav className="nav" aria-label="Main navigation">
          {['Create', 'Library', 'Templates'].map(item => <button key={item} className={activeTab === item ? 'nav-item active' : 'nav-item'} onClick={() => setActiveTab(item)}><span className={`nav-icon ${item.toLowerCase()}`} />{item}</button>)}
        </nav>
      ...
      <main className="main-content">
        ...
        {activeTab === 'Create' && <section className="create-view"><div className="eyebrow"><Sparkle small /> AI VIDEO STUDIO</div><h1>Turn your ideas<br /><span>into motion.</span></h1><p className="intro">Describe a scene, a feeling, or a story. AZ brings it to life in seconds.</p>
          <div className="generator"><div className="prompt-wrap"><textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="A slow aerial shot of a quiet coastal town at golden hour..." /><div className="prompt-footer"><span className="prompt-hint"><Sparkle small /> Tip: be descriptive about movement, mood, and light</span><span className="char-count">{prompt.length} / 500</span></div></div><div className="generator-options"><div className="option-group"><label>Format</label><div className="segmented">{['16:9', '9:16', '1:1'].map(value => <button key={value} onClick={() => setAspect(value)} className={aspect === value ? 'selected' : ''}>{value}</button>)}</div></div><div className="option-group"><label>Duration</label><button className="select">8 seconds <span>⌄</span></button></div><button className="generate" onClick={generateVideo} disabled={!prompt.trim() || isGenerating}>{isGenerating ? 'Creating...' : <><Sparkle small /> Generate video</>}</button></div></div>
      ...
      {showPricing && <Pricing onClose={() => setShowPricing(false)} onActivate={activatePlan} currentPlan={selectedPlan} />}
    </div>
  )
}
...
function Pricing({ onClose, onActivate, currentPlan }) { return <div className="modal-backdrop"><div className="pricing-modal"><button className="close" onClick={onClose}>×</button><div className="eyebrow">MAKE MORE MAGIC</div><h2>Choose your creative pace.</h2><p>Start free, upgrade when your ideas need more room.</p><div className="plans"><div className="plan"><small>CREATOR</small><strong>$19<em>/ month</em></strong><p>50 videos per month</p><button onClick={() => onActivate('Creator')}>{currentPlan === 'Creator' ? 'Current plan' : 'Choose Creator'}</button></div><div className="plan featured"><span className="popular">MOST POPULAR</span><small>STUDIO</small><strong>$49<em>/ month</em></strong><p>200 videos per month</p><button onClick={() => onActivate('Studio')}>{currentPlan === 'Studio' ? 'Current plan' : 'Choose Studio'}</button></div></div><p className="checkout-note">Demo checkout is enabled. Connect Stripe or Paddle before accepting real payments.</p></div></div> }
